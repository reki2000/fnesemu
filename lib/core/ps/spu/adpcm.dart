...
extension VoiceAdpcmDecoder on Voice {
  void decodeBlock() {
    ...
    for (int i = 0; i < Voice.blockOldSize; i++) {
      _block[i] = _block[i + Voice.blockSize];
    }

    for (int i = Voice.blockOldSize;
        i < Voice.blockSize + Voice.blockOldSize;
        i++) {
      ...
      if (log) {
        debugLog(
            "SPU${no.decimal2z}: decode:$i raw:${raw.hex8} shifted:$shifted filtered:$filtered val:$val");
      }
    ...
    if (log) {
      debugLog(
          "SPU${no.decimal2z}: addr:${_addr.hex24} block:${_block.map((e) => e.hex16).join(" ")}");
      logCounter--;
    }
  }

  /// from https://psx-spx.consoledev.net/soundprocessingunitspu/#4-point-gaussian-interpolation
  static const gaussianTable = [
    ...
    0x01A8, 0x01B4, 0x01C0, 0x01CC, 0x01D9, 0x01E5, 0x01F2, 0x0200, // 0x080
    ...
    0x1307, 0x1347, 0x1388, 0x13C9, 0x140B, 0x144D, 0x1490, 0x14D4, // 0x100
    ...
    0x3E4C, 0x3EA9, 0x3F05, 0x3F62, 0x3FBD, 0x4019, 0x4074, 0x40D0, // 0x180
  ...
  int gaussian(int v0, int v1, int v2, int v3, int index) {
    int v = (gaussianTable[0xff - index] * v3) +
