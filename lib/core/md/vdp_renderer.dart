import 'dart:typed_data';

import 'package:fnesemu/util/int.dart';
import 'package:fnesemu/util/uint8list.dart';

import 'vdp.dart';

// preliminary building an RGBA color map for all 512 colors
const _map3to8 = [0x00, 0x24, 0x49, 0x6d, 0x92, 0xb6, 0xdb, 0xff]; //

final Uint32List rgba = Uint32List.fromList(
  List.generate(512, (i) {
    final r = _map3to8[i & 0x07];
    final g = _map3to8[i >> 3 & 0x07];
    final b = _map3to8[i >> 6 & 0x07];
    return 0xff000000 | (b << 16) | (g << 8) | r;
  }, growable: false),
);

class _Tile {
  int no = 0;
  int nameAddrBase = 0;
  int hScroll = 0;
  int yOffsetInTile = 0; // scroll added
  int tileY = 0;
  int pattern = 0; // 32 bit c3c2c1c0 * 8
  bool prior = false;
  int hFlipXor = 0;
  int palette = 0; // pp0000

  int fetchedHHigh = -1;

  _Tile(
      this.no, this.nameAddrBase, this.hScroll, this.yOffsetInTile, this.tileY);
}

class Sprite {
  late final int x;
  late final int y;
  late final int patternAddr;
  late final int paletteNo;
  late final bool vFlip;
  late final bool hFlip;
  late final int height;
  late final int width;
  late final int vCells;
  late final bool priority;
  late final int next;

  int pattern = 0;
  int fetchedX2 = -1;

  Sprite.of(int d0, int d1, int d2, int d3) {
    y = d0 & 0x3ff;
    x = d3 & 0x1ff;

    vFlip = d2 & 0x1000 != 0;
    hFlip = d2 & 0x0800 != 0;
    priority = d2 & 0x8000 != 0;
    paletteNo = d2 >> 9 & 0x30;
    patternAddr = d2 << 5 & 0xffe0;

    next = d1 & 0x7f;

    vCells = (d1 >> 8 & 3) + 1;
    height = vCells << 3;

    final hCells = (d1 >> 10 & 3) + 1;
    width = hCells << 3;
  }
}

class PriorityColor {
  final int color;
  final bool isDirect;
  final bool isPrior;

  bool get isVisible => color & 0x0f != 0;

  const PriorityColor(this.color, {this.isDirect = false, this.isPrior = true});
}

extension VdpRenderer on Vdp {
  static int bitsBgWidth =
      0; // shift for scnaline(doubled in interlace mode 2) to tile_y
  static int maskBgWidth = 0; // mask for tile_x
  static int maskScrollH = 0; // mask for hscroll+hcounter
  static int maskScrollV = 0; // mask for vscroll+vcounter
  static int maskTileYOffset = 0x07;
  static int bitsTileV = 3; // shift for vscroll+vcounter to tile_y

  static int y = 0;
  static int yy = 0; // doubled when interlace mode

  static final spriteBuf =
      List<Sprite>.filled(20, Sprite.of(0, 0, 0, 0), growable: false);
  static int spriteBufIndex = 0;

  static final sprite0 = List<bool>.filled(32, false); // x of sprite 0

  static int max = 0;

  _fillSpriteBuffer() {
    spriteOverflow = false;

    final overflowCount = h32 ? 16 : 20;
    final displayCount = h32 ? 64 : 80;
    final pixelCount = h32 ? 256 : 320;

    final baseAddr = reg[5] << 9 & (h32 ? 0xfe00 : 0xfc00);
    int spriteNo = 0;
    int pixelsPerLine = 0;
    spriteBufIndex = 0;

    for (int i = 0; i < displayCount; i++) {
      final base = baseAddr + spriteNo * 8;
      final sp = Sprite.of(
          vram.getUint16BE(base.mask16),
          vram.getUint16BE((base + 2).mask16),
          vram.getUint16BE((base + 4).mask16),
          vram.getUint16BE((base + 6).mask16));

      final isTarget = isInterlaced
          ? (sp.y - 256 <= yy && yy < sp.y - 256 + sp.height * 2)
          : (sp.y - 128 <= y && y < sp.y - 128 + sp.height);

      if (isTarget) {
        if (spriteBufIndex == overflowCount) {
          spriteOverflow = true;
          break;
        }

        pixelsPerLine += sp.width;
        if (pixelsPerLine >= pixelCount) {
          break;
        }

        spriteBuf[spriteBufIndex++] = sp;
      }

      if (sp.next == 0) {
        break;
      }

      spriteNo = sp.next;
    }
  }

  PriorityColor _spriteColor() {
    for (int i = 0; i < spriteBufIndex; i++) {
      final sp = spriteBuf[i];
      final hh = hCounter + 128 - sp.x;

      if (0 <= hh && hh < sp.width) {
        final flippedX = sp.hFlip ? sp.width - hh - 1 : hh;
        final x1 = flippedX & 0x07;
        final x2 = flippedX >> 3;

        // fetch pattern data if x2 is changed
        if (x2 != sp.fetchedX2) {
          sp.fetchedX2 = x2;

          final yShift1 = isInterlaced ? 1 : 0;

          final vv = isInterlaced ? (yy + 256 - sp.y) : (y + 128 - sp.y);
          final flippedY = sp.vFlip ? (sp.height << yShift1) - vv - 1 : vv;
          final y1 = flippedY & maskTileYOffset;
          final y2 = flippedY >> bitsTileV;

          final addr = (sp.patternAddr << yShift1) +
              (((x2 * sp.vCells + y2) << bitsTileV | y1) << 2);
          sp.pattern = vram.getUint32BE(addr.mask16);
        }

        final shift = 7 - x1;
        final colorNo = (sp.pattern >> (shift << 2)) & 0x0f;

        if (colorNo > 0) {
          return PriorityColor(sp.paletteNo | colorNo, isPrior: sp.priority);
        }
      }
    }

    return const PriorityColor(0, isPrior: false);
  }

  _setBgSize() {
    bitsBgWidth = [5, 6, 7, 7][reg[16] & 0x03];
    maskBgWidth = (1 << bitsBgWidth) - 1;
    maskScrollH = (1 << (bitsBgWidth + 3)) - 1;

    bitsTileV = isInterlaced ? 4 : 3;
    maskTileYOffset = (1 << bitsTileV) - 1;

    final maskBgHeight = [0x1f, 0x3f, 0x7f, 0x7f][reg[16] >> 4 & 0x03];
    maskScrollV = maskBgHeight << bitsTileV | maskTileYOffset;
  }

  PriorityColor _windowColor(_Tile ctx) {
    final v = y;
    final hLow = hCounter & 0x07;
    final hHigh = hCounter >> 3;

    if (hHigh != ctx.fetchedHHigh) {
      ctx.fetchedHHigh = hHigh;
      // fetch pattern
      final name = ctx.nameAddrBase |
          (width == 256
              ? (hHigh & 0x1f) << 1 | (v >> 3) << 6
              : (hHigh & 0x3f) << 1 | (v >> 3) << 7);

      final d0 = vram[name];
      final d1 = vram[name.inc];

      ctx.prior = d0.bit7;
      ctx.palette = d0 >> 1 & 0x30;
      ctx.hFlipXor = d0.bit3 ? 0 : 7;
      final vFlipXor = d0.bit4 ? maskTileYOffset : 0;

      final offset = ctx.yOffsetInTile ^ vFlipXor;
      final addr = ((d0 << 8 & 0x0700 | d1) << bitsTileV | offset) << 2;

      ctx.pattern = vram.getUint32BE(addr.mask16);
    }

    final shift = hLow ^ ctx.hFlipXor;
    return PriorityColor(ctx.palette | (ctx.pattern >> (shift << 2)) & 0x0f,
        isPrior: ctx.prior);
  }

  PriorityColor _planeColor(_Tile ctx) {
    final h = (hCounter - ctx.hScroll) & maskScrollH;
    final xOffsetInTile = h & 0x07;
    final tileX = h >> 3;

    if (tileX != ctx.fetchedHHigh) {
      ctx.fetchedHHigh = tileX;
      // fetch pattern
      final name = ctx.nameAddrBase |
          (tileX & maskBgWidth | ctx.tileY << bitsBgWidth) << 1;

      final d0 = vram[name];
      final d1 = vram[name.inc];

      ctx.prior = d0.bit7;
      ctx.palette = d0 >> 1 & 0x30;
      ctx.hFlipXor = d0.bit3 ? 0 : 7;
      final vFlipXor = d0.bit4 ? maskTileYOffset : 0;

      final offset = ctx.yOffsetInTile ^ vFlipXor;
      final addr = ((d0 << 8 & 0x0700 | d1) << bitsTileV | offset) << 2;

      ctx.pattern = vram.getUint32BE(addr.mask16);
    }

    final shift = xOffsetInTile ^ ctx.hFlipXor;
    return PriorityColor(ctx.palette | (ctx.pattern >> (shift << 2)) & 0x0f,
        isPrior: ctx.prior);
  }

  // true: require rendering+hsync, false: retrace
  bool renderLine() {
    vCounter++;
    if (vCounter == Vdp.height + Vdp.retrace) {
      vCounter = 0;
      if (interlaceMode != 0) {
        oddFrame = !oddFrame;
      }
    }

    y = vCounter - 2;
    yy = interlaceMode != 3 ? y : ((y << 1) + (oddFrame ? 1 : 0));

    hBlank = false;

    if (y == -1) {
      busZ80.deassertInt();
    }

    final requireRender = 0 <= y && y < Vdp.height;

    if (isDmaRunning) {
      execDma(requireRender ? 9 : 102);
    }

    if (requireRender) {
      _setBgSize();
      _fillSpriteBuffer();

      final hScrollBase = reg[13] << 10 & 0xfc00;
      final isHScrollFull = !reg[11].bit1;
      final isHScrollLine = reg[11].bit0;
      final hScrollAddr = hScrollBase +
          (isHScrollFull
              ? 0
              : isHScrollLine
                  ? (y << 2)
                  : ((y & ~0x07) << 2));

      final isVFullScroll = !reg[11].bit3;
      final vScrollAddr = isVFullScroll ? 0 : (y >> 3 & ~0x01);

      final vScrollA = (yy + vsram[vScrollAddr]) & maskScrollV;
      final ctxA = _Tile(
          0, //
          reg[2] << 10 & 0xe000,
          vram.getUint16BE(hScrollAddr) & 0x3ff,
          vScrollA & maskTileYOffset,
          vScrollA >> bitsTileV);

      final vScrollB = (yy + vsram[vScrollAddr.inc]) & maskScrollV;
      final ctxB = _Tile(
          1, //
          reg[4] << 13 & 0xe000,
          vram.getUint16BE(hScrollAddr.inc2) & 0x3ff,
          vScrollB & maskTileYOffset,
          vScrollB >> bitsTileV);

      final ctxWindow = _Tile(
          2, // window
          reg[3] << 10 & 0xf800,
          0,
          yy & maskTileYOffset,
          yy >> bitsTileV);

      final windowH = reg[0x11].bit7
          ? [0, reg[0x11] << 4 & 0x1f0]
          : [reg[0x11] << 4 & 0x1f0, width];

      final windowV = reg[0x12].bit7
          ? [0, reg[0x12] << 3 & 0xf8]
          : [reg[0x12] << 3 & 0xf8, Vdp.height];

      final vInWindow = windowV[0] <= y && y < windowV[1];
      final bufferOffset = y * width;
      final bg = reg[7] & 0x3f;

      for (hCounter = 0; hCounter < width; hCounter++) {
        int color;

        final sprite = _spriteColor();
        if (sprite.isPrior && sprite.isVisible) {
          color = sprite.color;
        } else {
          final inWindow =
              windowH[0] <= hCounter && hCounter < windowH[1] && vInWindow;
          final planeAW =
              inWindow ? _planeColor(ctxA) : _windowColor(ctxWindow);
          if (planeAW.isPrior && planeAW.isVisible) {
            color = planeAW.color;
          } else {
            final planeB = _planeColor(ctxB);

            color = (planeB.isPrior && planeB.isVisible)
                ? planeB.color
                : sprite.isVisible
                    ? sprite.color
                    : planeAW.isVisible
                        ? planeAW.color
                        : planeB.isVisible
                            ? planeB.color
                            : bg;
          }
        }

        buffer[bufferOffset + hCounter] = rgba[cram[color]];
      }

      vBlank = false;
      return true;
    }

    vBlank = true; // during vblank
    return false;
  }

  void finishLine() {
    if (y == Vdp.height - 1 && enableVInt) {
      vBlank = true;
      status |= Vdp.bitVblankInt; // on: vsync int occureed
      bus.interrupt(6);
      busZ80.assertInt();
      return;
    }

    if (!vBlank) {
      hBlank = true;

      if (enableHInt) {
        if (hIntCounter == 0) {
          hIntCounter = reg[10];
          bus.interrupt(4);
          return;
        }

        hIntCounter--;
      }

      return;
    }

    if (vCounter == 0 || y >= Vdp.height) {
      hIntCounter = reg[10];
    }
  }
}
